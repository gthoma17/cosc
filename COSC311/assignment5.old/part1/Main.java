    ...
    public static void main(String[] args){
        ...
        while(option != 8){
            if (option == 1){
                ...
                sd.insert(Integer.valueOf(parms[0].trim()),parms[1],Integer.valueOf(parms[2].trim()),Double.valueOf(parms[3].trim()));
            }
            else if (option == 2){sd.delete(Integer.valueOf(getParm("id").trim()));}
            else if (option == 3){sd.search(Integer.valueOf(getParm("id").trim()));}
            else if (option == 4){
                String[] parms = getParm("stuData").split("\\s+");
                sd.modify(Integer.valueOf(parms[0].trim()),parms[1],Integer.valueOf(parms[2].trim()),Double.valueOf(parms[3].trim()));
            }
